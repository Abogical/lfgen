...
argument_parser = argparse.ArgumentParser(
    prog='quilter',
    description='Generates a light field quilt from a folder of images',
)
...
def main():
    ...
    for filename in os.listdir(arguments.directory):
        ...
        if match is None:
            warnings.warn(f'Filename {filename} does not match quilter format. Ignoring.', RuntimeWarning)
        else:
    ...
    if max_x == -1:
        ...
        raise SystemExit(f"No filename that matches the quilter format is inside the directory {arguments.directory}")
